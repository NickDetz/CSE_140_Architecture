// Code Nick Deters May 24th 2020
...
//Helper structs used for extra storage
struct addy {
    unsigned int offset;
    unsigned int index;
    unsigned int tag;
...
//Helper functions
// Will convert the structs in addy to proper cache flows
struct addy getBytes(address addr) {
    unsigned int off = 0;
    unsigned int ind = 0;
    unsigned int tag = 0;
    /*Block will only consist of 4,8,16,32 bits
     this will set the off unassigned integer to
     either 2,3,4,5 depending on how big it is
     */
    if (block_size == 4) {
        off = 0x3 & addr;
        addr = addr >> 2;
    ...
    /* This sets a counter to follow the policy of whicheer policy is used in the project, basically a holding area
     */
    if (set_count == 1) {
        ind = 0;
    }
...
/* In the LRU policy we need to make a function that will replace in the cache the least recently used policy.
   The LRU will come with a swiitch case that if used will cycle through the data and find the highest index or the Least recently used one being the highest index and replace it
 */
...
void accessMemory(address addr, word* data, WriteEnable we){
    ...
    /* Started by deciding to use structs in my code since it was
     easier to convert and mold into the program allowing me to see the
     proper right backs*/
    
    
    struct addy theAddr = getBytes(addr);
    
    // Declared the variables into TIO + added a hit boolean for efficiency
